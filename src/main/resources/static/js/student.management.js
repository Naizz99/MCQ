	...
	$.ajax({
		...
		success: function (answeredPaper) {
			...
			for (let x in answeredPaper["questionList"]){
				...
				var StudentPaperReviewDetailDto = {
					...
					reviewId	: document.getElementById("sprId").value,
					result	: 0,
					status	:document.getElementById(index).checked
				}
				
				$.ajax({
					url: '/secure/saveReviewDetail',
					type: 'POST',
					data: StudentPaperReviewDetailDto,
					cache: false,
			...
			if((correctQues != 0) && (correctQues == allQues)){
				Swal.fire({
				  ...
				  imageAlt: 'Custom image',
				  	...
				  	window.location = "/reviewPaper?reviewId="+document.getElementById("sprId").value;
				})
				...
				Swal.fire({
				  ...
				  imageAlt: 'Custom image',
				  	...
				  	//window.location = "/student/listPapersByPublisher?subjectId=" + sub + "&poId=" + publisher;
				  	window.location = "/reviewPaper?reviewId="+document.getElementById("sprId").value;
				})
...
function tryagain(){
  clicked = false;
  if((document.getElementById("availableAtp").value) >= 1){
	...
	clicked = true;
	Swal.fire('Total Attepmts has been exceed!');
  }
